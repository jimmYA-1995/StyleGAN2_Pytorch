...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d(input, self.kernel, up=1, down=self
                        .factor, pad=self.pad)
...
class Dense_layer(nn.Module):
    def __init__(self, in_dim, out_dim,
                 use_bias=True, bias_init=0,
                 nonlinearity='fused_lrelu', alpha=0.2,
    ...
    def __repr__(self):
        return (
            f'{self.__class__.__name__}({self.out_dim}, {self.in_dim} '
    ...
    def forward(self, x):
        ...
        if self.act.startswith('fused'):
            return activation_funcs[self.act](x, self.b * self.lrmul)
        if self.use_bias:
...
class Conv2d_layer(nn.Module):
    ...
    def __repr__(self):
        return (
            f'{self.__class__.__name__}({self.out_channel}, {self.in_channel}, {self.kernel} '
            f'mode={self.mode}'
        )    
...
class Modulated_conv2d_layer(nn.Module):
    def __init__(self, in_channel, out_channel, dlatents_dim,
                 kernel=3, mode=None, demodulate=True, resample_kernel=[1,3,3,1],
                 gain=1, use_wscale=True, lrmul=1):
    ...
    def __repr__(self):
        return (
            f'{self.__class__.__name__}({self.out_channel}, '
    ...
    def __init__(self, in_channel, out_channel,
        ...
        if use_modulate:
            self.conv = Modulated_conv2d_layer(
                in_channel, out_channel, dlatents_dim, **kwargs)
