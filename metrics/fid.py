...
class FIDTracker():
    def __init__(self, cfg, rank, num_gpus, out_dir, use_tqdm=False):
        ...
        if num_gpus == 1:
            ...
            self.inceptionV3 = load_patched_inception_v3().eval().to(self.device)
            # with open('/home/u3534611/.cache/dnnlib/downloads/a866f8d678872dcf6fcf60ddd09807ab_https___nvlabs-fi-cdn.nvidia.com_stylegan2-ada-pytorch_pretrained_metrics_inception-2015-12-05.pt', 'rb') as f:
                # self.inceptionV3 = torch.jit.load(f).eval().to(self.device)
            if self.rank == 0:
                torch.distributed.barrier()
    ...
    def calc_fid(self, generator, k_iter, save=False, eps=1e-6):
        ...
        self.log.info(f'FID in {str(1000 * k_iter).zfill(6)} \
             iterations: "{fids}". [costs {round(total_time, 2)} sec(s)]')
    ...
    def extract_feature_from_real_images(self, cfg):
        for i in range(self.num_classes):
            ...
            dataloader = torch.utils.data.DataLoader(
                dataset=dataset, sampler=item_subset, batch_size=4, num_workers=2, worker_init_fn=seed_worker)
            ...
            for _ in progress_bar:
                ...
                # feature = self.inceptionV3(torch.clamp(imgs * 127.5 + 127.5, 0, 255).to(torch.uint8).to(self.device), return_features=True)
